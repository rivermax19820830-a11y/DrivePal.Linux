...
{
	public class TrolleyCalculationStrategy : IMechanismCalculationStrategy
	{
		TrolleyGantryMechanism mechanism;

        public void Calculate(IMechanism entity)
		{
            mechanism = entity as TrolleyGantryMechanism ?? throw new ArgumentNullException(nameof(entity), "entity 不能转换为 TrolleyGantryMechanism");

            // 计算净功率
            decimal totalNetPower = CalculateTotalNetPower(mechanism.FrictionCoefficient, mechanism.LoadWeight_t, mechanism.SpreaderWeight_t, mechanism.TravelingSpeed_m_per_min, mechanism.TransmissionEfficiency);
            decimal singleNetPower = CalculateSingleNetPower(mechanism.IsMultidrive, mechanism.InverterCount, totalNetPower);
        ...
        public decimal CalculateTotalNetPower(decimal frictionCoefficient,  decimal loadWeight, decimal spreaderWeight, decimal runningSpeed, decimal transmissionEfficiency)
        {
            decimal totalNetPower = frictionCoefficient * (loadWeight + spreaderWeight) * runningSpeed / IndustryCoefficient / transmissionEfficiency;
